...
func GetDepartmentList(c *gin.Context) {
	...
	PS.Sort, PS.SortAr = BindSort( c.Query("sort") ,m.DepartmentMod{})
...
func UpdateDepartment(c *gin.Context) {
	...
	if c.BindJSON(&ftem) == nil {
		k1, errr := m.GetDepartment(ftem["_id"].(string))
		if errr != nil {
			log.Fatalln(errr)
...
func DeleteDepartment(c *gin.Context) {
	...
	if c.BindJSON(&ftem) == nil {
		fmt.Println(ftem)
		notexist, err := m.TestDepartment(map[string]interface{}{
		...
		if err != nil {
			...
			if !notexist {
				_, err := m.DeleteDepartment(ftem["_id"].(string))
				if err != nil {
					log.Println(err)
					RespondJSONWithError(c, 500, err)
				} else {
